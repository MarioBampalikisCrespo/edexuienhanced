class Netstat {
    constructor(parentId) {
        ...
        // Init GeoIP integrated backend (opened once in preload.js)
        this.geoLookup = {
            get: ip => window.api.geoip.get(ip)
        };
        window.api.geoip.ready.then(() => {
        ...
        window.si.networkInterfaces().then(async data => {
            ...
            if (net.ip4 === "127.0.0.1") {
                ...
                if (this.runsBeforeGeoIPUpdate === 0 && this.lastconn.finished) {
                    window.api.net.getExternalIP(net.ip4).then(data => {
                        this.ipinfo = {
                            ip: data.ip,
                        ...
                        console.warn(e);
                        window.api.log("note", "NetStat: Error parsing data from myexternalip.com");
    ...
    ping(target, port, local) {
        return window.api.net.ping(target, port, local);
    }
}
