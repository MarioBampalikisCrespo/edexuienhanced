    ...
    constructor(parentId) {
        ...
        window.si.cpu().then(data => {
            ...
            innercontainer.innerHTML = `<h1>CPU USAGE<i>${cpuName}</i></h1>
                ...
                <div>
                    <div>
                        <h1>${(window.api.process.platform === "win32") ? "CORES" : "TEMP"}<br>
                        <i id="mod_cpuinfo_temp">${(window.api.process.platform === "win32") ? data.cores : "--°C"}</i></h1>
                    </div>
                    <div>
                        <h1>SPD<br>
            ...
            this.updateCPUload();
            if (window.api.process.platform !== "win32") {this.updateCPUtemp();}
